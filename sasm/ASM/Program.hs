...
data Program = Program
  { codeSeg   ::  (Word16, DVS.Vector Word8)
  , dataSeg   ::  (Word16, DVS.Vector Word8)
  , otherSegs :: [(Word16, DVS.Vector Word8)]
  } deriving (Eq, Generic)

instance Show Program where
  show (Program (_, cdat) _ _) = header ++ foldMap (++ "\n") (showRow <$> rows)
    where header               = printf "    : 00 01 02 03 04 05 06 07 08 09 0a 0b 0c 0d 0e 0f\n"
          ...
          sl i len             = cdat & DVS.slice i len
          progLen              = DVS.length cdat
